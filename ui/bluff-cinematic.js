...
export async function mount(host,{eliminated,onImpact,isCancelled}){
 ...
 let frame=0,dead=false,impact=false;const started=performance.now();const resize=()=>{const w=Math.max(1,host.clientWidth),h=Math.max(1,host.clientHeight);renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();};const observer=new ResizeObserver(resize);observer.observe(host);resize();
 const smooth=t=>{t=Math.max(0,Math.min(1,t));return t*t*(3-2*t);};
 function draw(now){if(dead)return;if(isCancelled()){dispose();return;}const t=(now-started)/1000;
  const enter=smooth(t/.65),spin=smooth((t-.55)/1.25),aim=smooth((t-1.8)/.65);
  ...
  if(t>=2.5&&!impact){impact=true;onImpact();}
  const recoil=impact?Math.max(0,1-(t-2.5)/.25):0;pivot.rotation.x-=recoil*(eliminated?.2:.035);flash.intensity=eliminated&&t>=2.5&&t<2.58?7:0;
  renderer.render(scene,camera);frame=requestAnimationFrame(draw);
 }
 function dispose(){if(dead)return;dead=true;cancelAnimationFrame(frame);observer.disconnect();renderer.dispose();renderer.domElement.remove();}
