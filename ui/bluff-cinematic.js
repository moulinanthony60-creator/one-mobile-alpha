...
export async function mount(host,{eliminated,onImpact,onPhase=()=>{},getTargetRect,isCancelled}){
 ...
 const renderer=new T.WebGLRenderer({alpha:true,antialias:true,powerPreference:'low-power'});renderer.setPixelRatio(Math.min(devicePixelRatio||1,1.5));renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.15;
 renderer.domElement.setAttribute('aria-label','Revolver 3D anim̩');host.replaceChildren(renderer.domElement);
 ...
 gun.traverse(n=>{if(!n.isMesh)return;const tune=m=>{const copy=m.clone();materials.push(copy);const wood=/crosse|bois|grip/i.test(n.name+' '+m.name);copy.metalness=wood?.05:.88;copy.roughness=wood?.6:.29;if(!wood)copy.color.setHex(/vis|couronne|bande/i.test(n.name)?0xa9956c:0x64707a);else copy.color.setHex(0x59331f);return copy;};n.material=Array.isArray(n.material)?n.material.map(tune):tune(n.material);});pivot.add(gun);scene.add(pivot);const bounds=new T.Box3().setFromObject(gun),center=bounds.getCenter(new T.Vector3());gun.position.sub(center);
 ...
 function dispose(){if(dead)return;dead=true;cancelAnimationFrame(frame);observer.disconnect();materials.forEach(m=>m.dispose());table.geometry.dispose();table.material.dispose();rimGeo.dispose();rimMat.dispose();renderer.dispose();renderer.domElement.remove();}
