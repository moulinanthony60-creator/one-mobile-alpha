...
(()=>{
 let stop=()=>{},lastAction='',actionStarted=0;
 ...
 window.ONEBluffScene=(host,players,pending,game)=>{const signature=game.id+':'+game.revision;if(signature!==lastAction){lastAction=signature;actionStarted=performance.now();}
  ...
  const vs=`attribute vec3 pos;attribute vec3 normal;attribute vec3 color;attribute float motion;uniform float progress;uniform vec3 origin;uniform float aspect;uniform float yaw;uniform float pull;varying vec3 shade;
  void main(){float c=cos(yaw),s=sin(yaw);vec3 point=pos;if(motion>0.5&&motion<1.5){point+=origin*(1.0-progress);point.y+=sin(progress*3.14159)*.65;}if(motion>1.5){point.y+=sin(progress*3.14159)*.16;point.z-=sin(progress*3.14159)*.22;}vec3 p=vec3(c*point.x+s*point.z,point.y,-s*point.x+c*point.z);float y=.954*(p.y-3.0)-.300*(p.z-7.0);float d=-.300*(p.y-3.0)-.954*(p.z-7.0)+pull;gl_Position=vec4(p.x*2.8/aspect,y*2.8,d*1.002-0.2002,d);float light=.32+.68*max(dot(normal,normalize(vec3(-.45,1.,.55))),0.);shade=color*light;}`;
  ...
  try{
   ...
   motion=2;sphere(-1.10,.39,2.28,.21,.11,.37,[.60,.40,.29]);sphere(1.10,.39,2.28,.21,.11,.37,[.60,.40,.29]);
   motion=1;for(let i=0;i<(pending?.count||0);i++)box(-.18+i*.14,.26+i*.014,.10,.36,.01,.53,[.57,.31,.18]);
