...
(()=>{
 ...
 async function drain(){if(running)return;const key=window.oneProviderReady('youtube');if(!key){queue.clear();return;}running=true;
 try{while(queue.size){const ids=[...queue].slice(0,50);ids.forEach(id=>queue.delete(id));const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),12000);try{const res=await window.oneConnectedFetch('https://www.googleapis.com/youtube/v3/videos?part=status,contentDetails&id='+encodeURIComponent(ids.join(','))+'&key='+encodeURIComponent(key),{signal:controller.signal});if(!res.ok)throw new Error('verification');const data=await res.json();if(!Array.isArray(data.items))throw new Error('verification');const byId=new Map(data.items.map(v=>[v.id,v]));ids.forEach(id=>cache.set(id,{ok:allowed(byId.get(id)),until:Date.now()+ttl}));}catch{ids.forEach(id=>cache.set(id,{ok:false,unknown:true,until:Date.now()+60000}));}finally{clearTimeout(timeout);}}}
